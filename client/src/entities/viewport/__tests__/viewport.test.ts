...
import { bandasFor, idealDensity, strataFor } from '@/entities/viewport/math';

describe('viewport density math (spec 2.2)', () => {
  it('spec 3.4.2 view gives ideal 1, s 1, phi 0, 4 bandas', () => {
    ...
    expect(bandasFor(phi)).toBe(4);
  });
  it('bands follow phi quarters', () => {
    expect(bandasFor(0)).toBe(4);
    expect(bandasFor(0.3)).toBe(3);
    expect(bandasFor(0.6)).toBe(2);
    expect(bandasFor(0.9)).toBe(1);
  });
